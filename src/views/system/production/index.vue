<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="productionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="事故概述" align="center" prop="accidentOverview" />
     <el-table-column label="寻求何种帮助" align="center" prop="helpSeek">
             <template slot-scope="scope">
               <dict-tag :options="dict.type.help_type" :value="scope.row.helpSeek"/>
             </template>
           </el-table-column>
...
<script>
import { listProduction, getProduction, delProduction, addProduction, updateProduction } from "@/api/system/production";

export default {
  name: "Production",
  dicts: ['help_type','sys_check_status'],
  data() {
    return {
      titleVideo:'视频查看',
      //视频
...
this.openVideo= true;
this.dialogImageUrl= row.imageData
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加渔业生产、事故在线技术咨询、服务系统";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getProduction(id).then(response => {
        ...
        this.open = true;
        this.title = "修改渔业生产、事故在线技术咨询、服务系统";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除渔业生产、事故在线技术咨询、服务系统编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/production/export', {
        ...this.queryParams
