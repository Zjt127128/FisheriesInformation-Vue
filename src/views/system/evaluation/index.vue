        ...
        >修改</el-button>
			...
			<el-col :span="1.5">
				<el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple"
					@click="handleDelete" v-hasPermi="['system:evaluation:remove']">删除</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
        <el-button
          type="warning"
		...
		<el-table v-loading="loading" :data="evaluationList" @selection-change="handleSelectionChange">
			...
			<el-table-column label="身份类别" align="center" prop="identityCategory">
				<template slot-scope="scope">
					<dict-tag :options="dict.type.sf_id_type" :value="scope.row.identityCategory" />
				</template>
			</el-table-column>
			<el-table-column label="标的物的基本信息" align="center" prop="baseInformation" />
			<el-table-column label="申请后台给予信息支持" align="center" prop="applyFor" />
        ...
        <template slot-scope="scope">
          ...
          >审核</el-button>
					<el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
						v-hasPermi="['system:evaluation:remove']">删除</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
	import {
		...
		addEvaluation,
		updateEvaluation
	} from "@/api/system/evaluation";

	export default {
		name: "Evaluation",
		dicts: ['sf_id_type','sys_check_status'],
		data() {
			return {
				// 遮罩层
				loading: true,
		...
		methods: {
			...
			handleDelete(row) {
				...
				this.$modal.confirm('是否确认删除渔业资源资产价值评估系统编号为"' + ids + '"的数据项？').then(function() {
