<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:shipping:remove']"
        >删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="shippingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="作业水域" align="center" prop="jobArea" />
      <el-table-column label="船籍港" align="center" prop="homePort" />
      <el-table-column label="作业方式" align="center" prop="jobWay" />
      <el-table-column label="船长：宽：深" align="center" prop="shipLength" />
      <el-table-column label="材质" align="center" prop="materialQuality" />
      <el-table-column label="年油耗" align="center" prop="annualFuel" />
      <el-table-column label="证件是否齐全" align="center" prop="certificates">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_yes_no" :value="scope.row.certificates"/>
        </template>
      </el-table-column>
      <el-table-column label="附属设备是否齐全" align="center" prop="equipment">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_yes_no" :value="scope.row.equipment"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['system:shipping:edit']"
          >审核</el-button>
...
<script>
import { listShipping, getShipping, delShipping, addShipping, updateShipping } from "@/api/system/shipping";

export default {
  name: "Shipping",
  dicts: ['sys_yes_no','sys_check_status'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除渔业船舶技术经济资源信息共享系统编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/shipping/export', {
        ...this.queryParams
