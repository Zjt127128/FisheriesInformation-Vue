<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="ownerList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="关注并想获取何种帮助或信息" align="center" prop="helpInformation" >
        <template slot-scope="scope">
          <dict-tag :options="dict.type.help_type2" :value="scope.row.helpInformation"/>
        </template>
      </el-table-column>
...
export default {
  name: "Owner",
  dicts: ['id_type','help_type2','sys_check_status', 'sys_level'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加业主、专家、涉渔机构、在线交流共享系统";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getOwner(id).then(response => {
        ...
        this.open = true;
        this.title = "修改业主、专家、涉渔机构、在线交流共享系统";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除业主、专家、涉渔机构、在线交流共享系统编号为"' + ids + '"的数据项？').then(function() {
        return delOwner(ids);
      }).then(() => {
        this.getList();
