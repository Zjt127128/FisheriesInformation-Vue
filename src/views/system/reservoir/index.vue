<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:reservoir:remove']"
        >删除</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="reservoirList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="水域" align="center" prop="waters">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.water_type" :value="scope.row.waters"/>
        </template>
      </el-table-column>
      <el-table-column label="面积" align="center" prop="extent" >
        <template slot-scope="scope">
          {{scope.row.extent}}
          <dict-tag :options="dict.type.unit_type" :value="scope.row.extentDanwei"/>
        </template>
      </el-table-column>
      <el-table-column label="养殖物种类1" align="center" prop="breedingSpecies" />
      ...
      <el-table-column label="年产量" align="center" prop="annualOutput" >
        <template slot-scope="scope">
          {{scope.row.annualOutput}}
          <dict-tag :options="dict.type.unit_type2" :value="scope.row.annualOutputDanwei"/>
        </template>
      </el-table-column>
      <el-table-column label="年产量 规格" align="center" prop="annualOutputType" />
      <el-table-column label="卖价格" align="center" prop="sellingPrice" />
...
<!--      <el-table-column label="年投放量 规格" align="center" prop="annualReleaseTyle" />-->
      <el-table-column label="苗价格" align="center" prop="seedlingPrice" />
      <el-table-column label="年投放量 鲢：鳙：其他比例" align="center" prop="annualReleaseProportion" />
...
<script>
import { listReservoir, getReservoir, delReservoir, addReservoir, updateReservoir } from "@/api/system/reservoir";
import { regionData, CodeToText, TextToCode  } from 'element-china-area-data';
export default {
  name: "Reservoir",
  dicts: ['water_type','sys_check_status','fish_type', 'unit_type','unit_type2','breed_type'],
  data() {
    return {
	    options: regionData,
	    selectedOptions: [],
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加海底滩涂增养殖技术经济资源信息共享系统";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getReservoir(id).then(response => {
        ...
        this.open = true;
        this.title = "修改水库等大水面渔业技术经济资源信息状态";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除海底滩涂增养殖技术经济资源信息共享系统编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/reservoir/export', {
        ...this.queryParams
      }, `reservoir_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
